...
-   **`NodeExecutionError`**: The standard error thrown when a node fails after all retries. Additionally, it is thrown when dynamic nodes (e.g., those generated by `batch-scatter` or custom nodes) reference invalid implementations in the runtime's node registry, providing immediate fail-fast feedback for dynamically generated graphs. For subflows, this error includes detailed information from the failed subflow, including the specific node that failed and its stack trace.
...
When a subflow fails, the error is wrapped in a `NodeExecutionError` that includes detailed information from the subflow's execution. This helps with debugging by providing:
