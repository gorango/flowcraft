...
import EdgeLoopback from './Edge/Loopback.vue'
...
onMounted(() => {
	vueFlow.setNodes(vueFlowNodes)
	vueFlow.setEdges(vueFlowEdges)
})
...
eventBus.on('node:start', (event) => {
	...
	nodeData.value.set(nodeId, { ...currentData, status: 'pending', inputs: input })
})
...
<template>
	<div
		class="relative flex flex-col h-full rounded-lg overflow-hidden border border-border bg-background"
	>
		<header
			class="relative z-10 flex flex-wrap items-center gap-2 px-3 py-2 bg-card border-b border-border flex-shrink-0"
		>
			<button
				@click="runWorkflow"
				:disabled="isRunning"
				class="px-3 py-1 text-sm font-medium rounded-md bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
			>
			...
			<div v-if="awaitingNodes.length > 0" class="flex items-center gap-2">
				...
				<span class="text-sm font-medium text-muted-foreground">Resume:</span>
				<button
					v-for="nodeId in awaitingNodes"
					:key="nodeId"
					@click="resumeWorkflow(nodeId, { output: { approved: true } })"
					class="px-3 py-1 text-sm font-medium rounded-md bg-green-600 text-white hover:bg-green-700 transition-colors"
				>
					Approve
				</button>
				<button
					...
					class="px-3 py-1 text-sm font-medium rounded-md bg-red-600 text-white hover:bg-red-700 transition-colors"
				>
					Deny
				</button>
			</div>
			...
			<button
				@click="viewContext = !viewContext"
				v-if="executionResult"
				class="px-3 py-1 text-sm font-medium rounded-md border border-border hover:bg-muted transition-colors"
			>
		...
		<pre
			v-show="viewContext"
			class="absolute inset-0 top-[41px] z-20 overflow-auto bg-card/95 backdrop-blur-sm"
		>
			<code class="p-4 text-xs font-mono text-foreground block">{{ JSON.stringify(executionResult, null, 2) }}</code>
		</pre>
