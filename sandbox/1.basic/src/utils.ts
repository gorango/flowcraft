export async function callLLM(prompt: string): Promise<string> {
  ...
  - Future Implications`)
	...
	if (prompt.includes('paragraph') && prompt.includes('100 WORDS')) {
		...
		return Promise.resolve(`This section covers ${section} in detail. It's an important concept that helps us understand the broader implications. For example, consider how this applies in real-world scenarios. The key takeaway is that understanding this concept enables better decision-making and practical applications in various contexts.`)
	}

	if (prompt.includes('conversational') && prompt.includes('engaging')) {
		return Promise.resolve(`Have you ever wondered about this fascinating topic? Let me take you on a journey through these important concepts.
...
In conclusion, these concepts work together to create a comprehensive understanding that will serve you well in future endeavors.`)
	...
	return Promise.resolve(`Mock response for: ${prompt.substring(0, 100)}`)
}
