...
describe.skip('AzureQueueAdapter - Testcontainers Integration', () => {
	...
	beforeAll(async () => {
		...
		await retry(async () => {
			...
			await database.containers.createIfNotExists({ id: CONTEXT_CONTAINER, partitionKey: { paths: ['/runId'] } })
			await database.containers.createIfNotExists({ id: STATUS_CONTAINER, partitionKey: { paths: ['/runId'] } })
		})
