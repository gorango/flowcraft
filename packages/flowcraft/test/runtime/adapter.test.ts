...
describe('BaseDistributedAdapter', () => {
	...
	describe('Core Orchestration', () => {
		it('should execute a node and enqueue the next one in a linear flow', async () => {
			const job: JobPayload = { runId: 'run1', blueprintId: 'linear', nodeId: 'A' }
			const nodeB: NodeDefinition = { id: 'B', uses: 'output' }
			const edgeAB = { source: 'A', target: 'B' }

			vi.mocked(mockRuntime.executeNode).mockResolvedValue({ output: 'Result from A' })
			...
			expect(adapter.enqueueJob).toHaveBeenCalledWith({ runId: 'run1', blueprintId: 'linear', nodeId: 'B' })
		...
		it('should publish a "completed" result when a terminal "output" node finishes', async () => {
			const job: JobPayload = { runId: 'run1', blueprintId: 'linear', nodeId: 'B' }

			vi.mocked(mockRuntime.executeNode).mockResolvedValue({ output: 'Final Result' })
			...
			expect(adapter.publishFinalResult).toHaveBeenCalledWith('run1', expect.objectContaining({
				status: 'completed',
		...
		it('should terminate a branch without completing the workflow if a non-output terminal node is reached', async () => {
			...
			vi.mocked(mockRuntime.executeNode).mockResolvedValue({ output: 'end of branch' })
	...
	describe('Fan-In Join Logic', () => {
		it('should wait for all predecessors with "all" join strategy', async () => {
			const job: JobPayload = { runId: 'run1', blueprintId: 'fan-in', nodeId: 'A' }
			vi.mocked(mockRuntime.executeNode).mockResolvedValue({ output: 'from A' })
			vi.mocked(mockRuntime.determineNextNodes).mockResolvedValue([{ node: fanInBlueprint.nodes[2], edge: fanInBlueprint.edges[0] }])
		...
		it('should enqueue the job when the last predecessor arrives with "all" join strategy', async () => {
			const job: JobPayload = { runId: 'run1', blueprintId: 'fan-in', nodeId: 'B' }
			vi.mocked(mockRuntime.executeNode).mockResolvedValue({ output: 'from B' })
			vi.mocked(mockRuntime.determineNextNodes).mockResolvedValue([{ node: fanInBlueprint.nodes[2], edge: fanInBlueprint.edges[1] }])
			...
			expect(adapter.enqueueJob).toHaveBeenCalledWith({ runId: 'run1', blueprintId: 'fan-in', nodeId: 'C' })
		})

		it('should enqueue the job only for the first predecessor with "any" join strategy', async () => {
			const job: JobPayload = { runId: 'run1', blueprintId: 'fan-in-any', nodeId: 'A' }
			vi.mocked(mockRuntime.executeNode).mockResolvedValue({ output: 'from A' })
			vi.mocked(mockRuntime.determineNextNodes).mockResolvedValue([{ node: fanInAnyBlueprint.nodes[2], edge: fanInAnyBlueprint.edges[0] }])
			...
			expect(adapter.enqueueJob).toHaveBeenCalledWith({ runId: 'run1', blueprintId: 'fan-in-any', nodeId: 'C' })
		})

		it('should not enqueue the job for subsequent predecessors with "any" join strategy', async () => {
			const job: JobPayload = { runId: 'run1', blueprintId: 'fan-in-any', nodeId: 'B' }
			vi.mocked(mockRuntime.executeNode).mockResolvedValue({ output: 'from B' })
			vi.mocked(mockRuntime.determineNextNodes).mockResolvedValue([{ node: fanInAnyBlueprint.nodes[2], edge: fanInAnyBlueprint.edges[1] }])
	...
	describe('Error Handling', () => {
		...
		it('should publish a "failed" result if the blueprint is not found', async () => {
			const job: JobPayload = { runId: 'run1', blueprintId: 'non-existent', nodeId: 'A' }
